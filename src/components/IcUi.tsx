...
import { View, ViewStyle, TouchableOpacity, StyleSheet, TouchableWithoutFeedback, TextStyle } from 'react-native'
...
export const RadioButtonUi = ({ value, text, setValue, color, textStyle }: RadioButtonProps) => {
  return (
    <Row>
      <RadioButton
        value={value}
        status={value === text ? 'checked' : 'unchecked'}
        onPress={() => { setValue(text) }}
        color={color}
      />
      <MText style={[styles.radioText, textStyle]}>{text}</MText>
    </Row>
  )
}
